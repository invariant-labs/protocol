...
describe("ido-pool", () => {
  ...
  const watermelonIdoAmount = new anchor.BN(10000000);
  ...
  it("Initializes the state-of-the-world", async () => {
    usdcMintAccount = await createMint(provider,SOL_DECIMAL);
    watermelonMintAccount = await createMint(provider, INV_DECIMAL);
  ...
  let userUsdc = null;
  // 10 usdc
  const firstDeposit = new anchor.BN(12_000_000_000);
  ...
  // 23 usdc
  const secondDeposit = new anchor.BN(30_000_000_000);
  ...
  const firstWithdrawal = new anchor.BN(2_000_000_000);
  ...
  it("Exchanges second user's Redeemable tokens for watermelon", async () => {
    ...
    poolWatermelonAccount = await getTokenAccount(provider, poolWatermelon);
    secondUserWatermelonAccount = await getTokenAccount(provider, secondUserWatermelon);
