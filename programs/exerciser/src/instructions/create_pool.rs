...
pub struct CreatePool<'info> {
    #[account(init,
        seeds = [b"exerciser", option_token_mint.to_account_info().key.as_ref(), redeem_token_mint.to_account_info().key.as_ref()],
        bump, payer = founder
    )]
    ...
    #[account(mut,
        constraint = redeem_token_vault.mint == redeem_token_mint.key() 
    )]
    pub redeem_token_vault: Account<'info, TokenAccount>,
    #[account(mut)]
...
impl<'info> CreatePool<'info> {
    pub fn handler(&self, amount_in: u64, ratio: u64) -> ProgramResult {
        ...
        **pool = Pool {
            ...
            redeem_token_mint: *self.redeem_token_mint.to_account_info().key,
            invariant_mint: *self.invariant_mint.to_account_info().key,
            invariant_token_vault: *self.invariant_vault.to_account_info().key,
            redeem_token_vault: *self.invariant_vault.to_account_info().key,
            ratio,
        };
