...
describe('Multiple swap', () => {
  ...
  it('swap X to Y 10 times', async () => {
    ...
    const [user1XAccount, user1YAccount] = await Promise.all([
      tokenX.createAccount(user1.publicKey),
      tokenY.createAccount(user1.publicKey)
    ])
    await Promise.all([
      tokenX.mintTo(user1XAccount, mintAuthority.publicKey, [mintAuthority], tou64(mintAmount)),
      tokenY.mintTo(user1YAccount, mintAuthority.publicKey, [mintAuthority], tou64(mintAmount))
    ])
    ...
    const initPositionVars: InitPosition = {
      ...
      userTokenX: user1XAccount,
      userTokenY: user1YAccount,
      lowerTick,
      upperTick,
      liquidityDelta,
    ...
    const [user2AccountX, user2YAccount] = await Promise.all([
